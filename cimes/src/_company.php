...
$company =array(
  ...
  ,array(
    ...
    'href' => 'http://www.ares.com.tw/events/industry4-Everest-ciMes',
    'subgroup' => ''
  )
  ,array(
    'group' => 'others',
...
if($str=='' || preg_match('/(gclid|_ga|utm_|fbclid)/i', strtolower($str))){
  foreach ($company as $key) {
    $temp1=isset($key['img']) ? '<figure class="card-img-top text-center"><img class="img-fluid" src="'.$key['img'].'" alt="'.$key['name'].'"></figure>' : '<div class="text-center">'.$key['name'].'</div>';
    $temp2=isset($key['href']) ? '<div class="col-12 text-right"><a href="'.$key['href'].'" class="btn btn-cimes mb-3 mr-3" rel="noopener noreferrer" target="_blank">詳細內容</a></div>' : '';
    $temp3=isset($key['subgroup']) ? '<strong class="blue-color">'.$key['subgroup'].'</strong>' : '';
    $_html=$_html.'<li class="col-12 col-md-6 pb-3">
              <div class="card">
                <div class="d-flex flex-wrap justify-content-center align-items-center">
                  <div class="col-12 col-sm-5 col-md-4">'.$temp1.'</div>
                  <div class="col-12 col-sm-7 col-md-8">
                    <div class="card-body">
                      <div class="card-title dline h4">'.$key['name'].'</div>'.$temp3.'<p class="card-text">'.$key['text'].'</p>
                    </div>
                  </div>'.$temp2.'</div>
              </div>
            </li>';
  ...
  foreach ($company as $key) {
    if (strpos($key['group'], $str) !== false && $key['text'] !== '') {
      $temp1=isset($key['img']) ? '<figure class="card-img-top text-center"><img class="img-fluid" src="'.$key['img'].'" alt="'.$key['name'].'"></figure>' : '<div class="text-center">'.$key['name'].'</div>';
      $temp2=isset($key['href']) ? '<div class="col-12 text-right"><a href="'.$key['href'].'" class="btn btn-cimes mb-3 mr-3" rel="noopener noreferrer" target="_blank">詳細內容</a></div>' : '';
      $temp3=isset($key['subgroup']) ? '<strong class="blue-color">'.$key['subgroup'].'</strong>' : '';
      $_html=$_html.'<li class="col-12 col-md-6 pb-3">
