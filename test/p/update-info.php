...
$_html = '';
foreach ($rs as $row) {
  $_html = $_html.'<tr><td style="white-space:nowrap">'.$row['date'].'</td><td>'.$row['product'].'</td><td>'.$row['title'].'</td><td style="white-space:pre-wrap;word-break: break-all;overflow-wrap:break-word;">'.$row['link'].'</td><td class="d-none">'.$row['img'].'</td><td class="d-none">'.$row['alt'].'</td><td>'.$row['id'].'</td></tr>';
}
$rs=null;
$db=null;
?>
      ...
      <div class="row justify-content-center">
        ...
        <form id="contact_form" class="mt-3" name="contact_form" method="post" onsubmit="return formCheck();">
          <div class="input-group mb-3">
            <input type="text" class="form-control" name="qq" id="qq" value="info" disabled>
          </div>
          <div class="input-group mb-3">
...
$('#table tbody tr').on('click',function(){
  ...
  $('#product').val($td.eq(1).text());
  $('#title').val($td.eq(2).html());
  $('#link').val($td.eq(3).text());
  $('#author_img').val($td.eq(4).text());
...
// $('#qq').change(function(){
  ...
  if($table=='ind'){
    ...
    $('#date,#title,#link,#product,#author,#author_img').closest('.input-group').show();
  }
