...
$query = "SELECT * FROM ind ORDER BY `date` DESC";
$rs = $db->query($query)->fetchAll();
$_html = '';
foreach ($rs as $row) {
  $_html = $_html.'<tr><td style="white-space:nowrap">'.$row['date'].'</td><td>'.$row['title'].'</td><td style="white-space:pre-wrap;word-break: break-all;overflow-wrap:break-word;">'.$row['link'].'</td><td class="d-none">'.$row['img'].'</td><td class="d-none">'.$row['description'].'</td><td>'.$row['id'].'</td></tr>';
}
$rs=null;
$db=null;
?>
      ...
      <div class="row justify-content-center">
        ...
        <form id="contact_form" class="mt-3" name="contact_form" method="post" onsubmit="return formCheck();">
          <div class="input-group mb-3">
            <input type="text" class="form-control" name="qq" id="qq" value="ind" disabled>
          </div>
          <div class="input-group mb-3">
...
<?php include('all_products.php'); ?>
          ...
          <div><span class="emmet-powered" style="font-family: sans-serif;font-size: 11px;color: #0072bc;cursor: pointer;border-bottom: 1px dashed #0072bc;">Powered by Emmet</span>
