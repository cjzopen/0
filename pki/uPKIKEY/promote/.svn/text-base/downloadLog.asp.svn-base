...
<%
	...
	if sortby <> "" then
		select case sortby
			case "ip":
				sql = "select top 30 * from DLLog order by ip"
				sql2 = "select count(*) from DLLog"
			case "when":
				sql = "select top 30 * from DLLog order by when DESC"
				sql2 = "select count(*) from DLLog"
			case else:
				sql = "select top 30 * from DLLog order by id DESC"
				sql2 = "select count(*) from DLLog"
		end select
	else
		sql = "select top 30 * from DLLog order by id DESC"
	...
	if not rs.eof then
		...
		response.write "Welcome <b>" & loginName & "</b>, ARES uPKI KEY 體驗版總共下載筆數: <b>" & rs2(0) & "</b><br><br>"
		response.write "<div style='float:left;'>"
		response.write "系統顯示前 30 筆<table cellpadding='5' cellspacing='1'>"
	...
	Function GetPkiTime2BeforeMon()
		stoday = now
		stoday2 = DateAdd("m", -1, stoday)
		y = Year(stoday2)
		if Len(Month(stoday2)) < 2 then
			m = "0" & Month(stoday2)
		else
			m = Month(stoday2)
		end if
		GetPkiTime2BeforeMon = y & m
	End Function
	...
	<tr>
		...
		<td width="17" valign="bottom" align="center">
			<%
					...
					startDate = GetPkiTime2BeforeMon & d & "000000"
					endDate = GetPkiTime2BeforeMon & d & "999999"
					...
					if height = 0 then
			%>
			0<br/><img src="icon_11x3.gif" border="0" width="11" height="<%=height%>" style="visibility:hidden;"/>
			<%
					else
			%>
			<%=dCount%><br/>
