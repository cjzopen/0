  <footer>
    <a href="" class="back-to-top" title="Back to top" role="button">
      <svg xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="274 364 64 64" enable-background="new 274 364 64 64" xml:space="preserve"><path fill="#FFFFFF" d="M308.249,391L307,389.75l-10,10l2.5,2.5l7.501-7.499l7.5,7.499l2.5-2.5L308.249,391z"/></svg>
      <!-- <i class="fa fa-angle-up"></i> -->
    </a>
    <div class="container">
      <div class="row">
        <div class="col-xs-12 col-sm-6 col-md-3">
          <h5 class="blue-color"><a href="/profile/">關於資通</a></h5>
          <ul>
            <li><a href="/contact/">聯絡我們</a></li>
            <li><a href="/profile/">公司簡介</a></li>
            <li><a href="/vision/">經營理念</a></li>
            <li><a href="/milestone/">里程碑</a></li>
            <li><a href="/partners/">客戶及合作夥伴</a></li>
            <li><a href="/csr/">企業社會責任</a></li>
            <!-- <li><a href="/download/">資源下載</a></li> -->
          </ul>
        </div>
        <div class="col-xs-12 col-sm-6 col-md-3">
          <h5 class="blue-color"><a href="/job/">菁英招募</a></h5>
          <ul>
            <li><a href="/job/">資通文化</a></li>
            <li><a href="/job/#benefits">福利制度</a></li>
            <!-- <li><a href="/job/#development">學習成長</a></li> -->
            <li><a href="/job/#jobs">加入資通</a></li>
            <li><a href="/job/#article">相關文章</a></li>
          </ul>
        </div>
        <div class="clearfix hidden-md hidden-lg"></div>
        <div class="col-xs-12 col-sm-6 col-md-3">
          <h5 class="blue-color"><a href="/governance/">投資人專區</a></h5>
          <ul>
            <li><a href="/governance/">公司治理</a></li>
            <li><a href="/investor-service/">股東服務</a></li>
            <li><a href="/investor-events/">重大訊息</a></li>
            <li><a href="/investor-revenue/">營收報告</a></li>
            <li><a href="/investor-financial/">財務報表</a></li>
            <li><a href="/investor-report/">股東會資訊</a></li>
          </ul>
        </div>
        <div class="col-xs-12 col-sm-6 col-md-3">
          <h5 class="blue-color"><a href="/stakeholder/">利害人專區</a></h5>
          <ul>
            <li><a href="https://web.ares.com.tw/aresoa" accesskey="8" title="alt 8" rel="nofollow">員工專區</a></li>
            <!-- <li><a href="/partners/#partner">策略夥伴專區</a></li> -->
            <li><a href="/stakeholder/">利害人專區</a><br><br></li>
            <li class="social">
              <a href="https://www.facebook.com/AresInternational" rel="external" title="資通電腦粉絲專頁"><i class="fa fa-facebook"></i></a>
              <a href="https://www.youtube.com/user/AresTaiwan/?sub_confirmation=1" rel="external" title="資通 youtube 頻道"><i class="fa fa-youtube"></i></a>
              <a href="https://www.linkedin.com/company/ares-international-corporation" rel="external" title="資通 linkedin 頁面"><i class="fa fa-linkedin-square"></i></a>
            </li>
          </ul>
        </div>
      </div>
    </div>
    <div class="foot-small">
      <div class="container">
        <div class="row">
          <div class="col-sm-8">
            <small>版權所有 &copy; <?= date('Y'); ?> 資通電腦股份有限公司</small>
          </div>
          <div class="col-sm-4 text-right">
            <span>
              <a href="/sitemap/" accesskey="m">網站地圖</a> | <a href="/privacy/">隱私權聲明</a>
            </span>
          </div>
        </div>
      </div>
    </div>
  </footer>
  </div>
<script src="/js/main2016.js?2006" async></script>
<?php include_once "/sites/global/cookie-notification.php"; ?>
