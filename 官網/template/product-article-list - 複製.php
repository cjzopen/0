...
function list_html($date,$title,$description,$img,$href,$ind,$keyword){
  return '<div class="list-item" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" data-keyword="'.$keyword.'">
...
function case_list($product) {
  ...
  $article_html = '';
    ...
    $query = "SELECT news.id, news.date, news.title, news.content, news.redirect, news.news_name, news.img, news.description
      ...
      WHERE best_practice.product LIKE '%{$product}%'
      AND news.id = best_practice.news_id COLLATE NOCASE";
    ...
    //$query = "SELECT * FROM best_practice WHERE product='{$product}' AND href IS NOT NULL ORDER BY href DESC";
    $query = "SELECT * FROM news WHERE success_case=1 AND product LIKE '%{$product}%' COLLATE NOCASE";
    $result = $db->query($query);
    foreach ($result->fetchAll() as $row) {
      $lists[$i]['date']  = $row['date'];
      $lists[$i]['title'] = $row['title'];
