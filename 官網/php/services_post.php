...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  // // your secret key
  // $secret = "6LcizxAUAAAAAE0hxFnFAs74xNlo-xhBvxLKOrWh";
  // // empty response
  // $response = null;
  // // check secret key
  ...
  if(preg_match($pattern, $company) === 1 || preg_match($pattern, $department) === 1 || preg_match($pattern, $name) === 1 || preg_match($pattern, $jobTitle) === 1 || preg_match($pattern, $phone) === 1 || preg_match($pattern, $mobile) === 1 || preg_match($pattern, $otherRequire) === 1) {
    ...
    function sendmail($product, $recipients, $body) {
      ...
      $mail->isHTML(true);
      $mail->Encoding = "8bit";
      $mail->setFrom('Consult@ares.com.tw', 'Consult');
...
a, a:link a:hover{color:#2858aa;text-decoration: none;} img {border: 0; outline: 0; }#link {width: 400px;vertical-align: bottom;text-align: right;}.data {text-align: left;}.data td {border: 1px solid #989898;}.bg {background-color: #ebebeb;width:30%;}
#footer {text-align: right;margin-top: 0;font-size: 11px;color: #2858aa;}td {margin: 0;padding: 8px;}
strong { color: #2858aa; }#content {padding: 0;}#footer td { border-top: 2px solid #2858aa; }
--> </style>
<body>
<table>
  <tr>
    <td><h3 style="color:#2858aa;font-size: 1.5em;">{$product}</h3></td>
  </tr>
  <tr>
    <td id="link"><a href="{$link}">source</a></td>
  </tr>
  <tr>
    <td colspan="2" id="content">
      <img src="cid:banner" alt="資通電腦" width="500" height="140" style="display: block; border: 0;" />
    </td>
  </tr>
  <tr>
  ...
  <tr>
    <td colspan="2">
      <ul>
        <li>您是如何得知資通產品？：<strong>{$knowFromAll}</strong></li>
        <li>請問 貴公司預計導入費用為何：<strong>{$budget}</strong></li>
      </ul>
      <ul>
  ...
  <tr id="footer">
    <td colspan="2">{$product} 線上諮詢- {$submitDate}</td>
  </tr>
</table>
</body>
</html>
