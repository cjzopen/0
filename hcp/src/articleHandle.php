...
foreach ($rs as $row) {
  ...
  if(!empty($row['title'])&& !empty($row['lead'])&& !empty($row['url'])){
    $_html = $_html.'<article class="mb-4 mb-lg-0" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
    <a itemprop="item url" class="row" href="'.$row['url'].'" rel="external">
      <div class="col-3 col-sm-3 col-lg-3">
        <figure class="ratio-box"><img class="lazyload" src="/img/temp.jpg" data-src="'.$row['photo'].'" alt="'.$row['title'].'"></figure>
      ...
      <div class="col-9 col-sm-8 col-lg-8">
        ...
        <h3 itemprop="name" class="main-color mt-0 mb-3">'.strip_tags($row['title']).'</h3>
...
$rs=null;
$db=null;
echo !empty($_html) ? $_html: '<p>本年度沒有文章</p>';