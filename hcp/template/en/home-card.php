...
// 最新消息
$rs = $db->query("SELECT * FROM news WHERE products LIKE '%{$products}%' ORDER BY date DESC LIMIT 1");
...
$rs = $db->query("SELECT news.id, news.date, news.title, news.content, news.redirect, news.news_name, news.description, news.img
        FROM news, best_practice WHERE best_practice.products LIKE '%{$products}%' AND news.id = best_practice.news_id ORDER BY news.date DESC");
...
}elseif(!empty($best_practice['content']) && !empty($best_practice['news_name'])){
...
if (empty($best_practice['description'])){
  if (!empty($best_practice['content'])) {
    if(preg_match('/<ul class=\"summary-effect\">(.*?)<\/ul>/s',$best_practice['content'],$matches)){
...
// 知識分享
$rs = $db->query("SELECT * FROM articles WHERE products LIKE '%{$products}%' ORDER BY date DESC LIMIT 1");
$knowledge = $rs->fetch(PDO::FETCH_ASSOC);
if(!empty($knowledge['img'])){
  $knowledge['photo'] = 'https://www.ares.com.tw'.$knowledge['img'];
}else{
        ...
        <div class="row">
          <div class="col-12 col-sm-6 col-lg-3">
            <article class="card h-entry hentry">
              ...
              <a href="<?= $news['href']; ?>" class="u-url" rel="bookmark">
                <figure class="card-img-top ratio-box entry-thumbnail">
                  <img class="mx-auto lazyload" src="/img/temp.jpg" data-src="<?= @$news['photo'] ?>" alt="<?= strip_tags($news['title']); ?>">
                </figure>
              </a>
              <h3 class="category">最新消息</h3>
              <div class="card-body">
                  <!-- <time datetime=""></time> -->
                <h4 class="card-title sub-color p-name entry-title"><a href="<?= $news['href']; ?>" title="<?= strip_tags($news['title']); ?>"><?= strip_tags($news['title']); ?></a></h4>
                <p class="card-text mb-0 p-summary entry-summary"><a href="<?= $news['href']; ?>"><?= strip_tags($news['lead']); ?></a></p>
          ...
          <div class="col-12 col-sm-6 col-lg-3">
            <article class="card h-entry hentry">
              ...
              <a href="<?= $knowledge['href']; ?>" class="u-url" rel="bookmark">
                <figure class="card-img-top ratio-box entry-thumbnail">
                  <img class="mx-auto lazyload" src="/img/temp.jpg" data-src="<?= $knowledge['photo'] ?>" alt="<?= strip_tags($knowledge['title']); ?>">
                </figure>
              </a>
              <h3 class="category">知識分享</h3>
              <div class="card-body">
                <h4 class="card-title sub-color p-name entry-title"><a href="<?= $knowledge['href']; ?>" title="<?= strip_tags($knowledge['title']); ?>"><?= strip_tags($knowledge['title']); ?></a></h4>
                <p class="card-text mb-0 p-summary entry-summary"><a href="<?=$knowledge['href']; ?>"><?= strip_tags($knowledge['lead']); ?></a></p>
                <span class="grad"></span>
              </div>
            </article>
          </div>
          <div class="col-12 col-sm-6 col-lg-3">
            <article class="card h-entry hentry">
              ...
              <a href="<?= $best_practice['href']; ?>" class="u-url" rel="bookmark">
                <figure class="card-img-top ratio-box entry-thumbnail">
                  <img class="mx-auto lazyload" src="/img/temp.jpg" data-src="<?= @$best_practice['photo'] ?>" alt="<?= strip_tags($best_practice['title']); ?>">
                </figure>
              </a>
              <h3 class="category">成功案例</h3>
              <div class="card-body">
                <h4 class="card-title sub-color p-name entry-title"><a href="<?= $best_practice['href']; ?>" title="<?= strip_tags($best_practice['title']); ?>"><?= strip_tags($best_practice['title']); ?></a></h4>
                <a href="<?= $best_practice['href']; ?>" title="<?= strip_tags($best_practice['lead']); ?>" class="p-summary entry-summary"><?= $best_practice['lead']; ?></a>
                <span class="grad"></span>
              </div>
            </article>
          </div>
            ...
            <div class="card">
              <a href="/contact/" rel="nofollow">
                <figure class="card-img-top ratio-box">
                  <img class="mx-auto lazyload" src="/img/temp.jpg" data-src="/img/inquiry.jpg" alt="線上及電話諮詢">
                </figure>
              </a>
              <h3>免費諮詢</h3>
              <div class="card-body">
                <h4 class="card-title sub-color"><a href="/contact/" rel="nofollow">您是否想要進一步評估呢？</a></h4>
                <p class="card-text mb-0"><a href="/contact/" rel="nofollow">請立即在線上留下您的資料<br>或來電：02-2522-1351 ext. 301。<br>我們將盡快與您連絡，並安排專業顧問到貴公司提供免費的現場諮詢！</a></p>
                <span class="grad"></span>
              </div>
            </div>
          </div>
